...
class XmlSecurityTest < Test::Unit::TestCase
  include XMLSecurityFrozen081

  context "XmlSecurity" do
    setup do
      @document = XMLSecurityFrozen081::SignedDocument.new(Base64.decode64(response_document))
    ...
    should "should raise Fingerprint mismatch" do
      exception = assert_raise(OneLogin::RubySamlFrozen081::ValidationError) do
    ...
    should "correctly obtain the digest method with alternate namespace declaration" do
      document = XMLSecurityFrozen081::SignedDocument.new(fixture(:adfs_response_xmlns, false))
  ...
  context "Algorithms" do
    should "validate using SHA1" do
      @document = XMLSecurityFrozen081::SignedDocument.new(fixture(:adfs_response_sha1, false))
    ...
    should "validate using SHA256" do
      @document = XMLSecurityFrozen081::SignedDocument.new(fixture(:adfs_response_sha256, false))
    ...
    should "validate using SHA384" do
      @document = XMLSecurityFrozen081::SignedDocument.new(fixture(:adfs_response_sha384, false))
      assert @document.validate_document("F1:3C:6B:80:90:5A:03:0E:6C:91:3E:5D:15:FA:DD:B0:16:45:48:72")
    end

    should "validate using SHA512" do
      @document = XMLSecurityFrozen081::SignedDocument.new(fixture(:adfs_response_sha512, false))
      assert @document.validate_document("F1:3C:6B:80:90:5A:03:0E:6C:91:3E:5D:15:FA:DD:B0:16:45:48:72")
    end
  end
  ...
  context "XmlSecurity::SignedDocument" do
    ...
    context "#extract_inclusive_namespaces" do
      ...
      should_eventually 'support inclusive canonicalization' do
        ...
        response = OneLogin::RubySamlFrozen081::Response.new(fixture("tdnf_response.xml"))
    ...
    context "XMLSecurityFrozen081::DSIG" do
      should "sign a AuthNRequest" do
        ...
        request = OneLogin::RubySamlFrozen081::Authrequest.new.create_authentication_xml_doc(settings)
        request.sign_document(ruby_saml_key, ruby_saml_cert)

        # verify our signature
        signed_doc = XMLSecurityFrozen081::SignedDocument.new(request.to_s)
      ...
      should "sign a LogoutRequest" do
        ...
        request = OneLogin::RubySamlFrozen081::Logoutrequest.new.create_logout_request_xml_doc(settings)
        request.sign_document(ruby_saml_key, ruby_saml_cert)
      ...
      should "sign a LogoutResponse" do
        ...
        response = OneLogin::RubySamlFrozen081::SloLogoutresponse.new.create_logout_response_xml_doc(settings, 'request_id_example', "Custom Logout Message")
        response.sign_document(ruby_saml_key, ruby_saml_cert)

        # verify our signature
        signed_doc = XMLSecurityFrozen081::SignedDocument.new(response.to_s)
        signed_doc.validate_document(ruby_saml_cert_fingerprint, false)
      end
    end
    ...
    context "StarfieldTMS" do
      setup do
        @response = OneLogin::RubySamlFrozen081::Response.new(fixture(:starfield_response))
        @response.settings = OneLogin::RubySamlFrozen081::Settings.new(
