...
class SloLogoutresponseTest < Test::Unit::TestCase
  ...
  context "SloLogoutresponse" do
    ...
    should "create the deflated SAMLResponse URL parameter" do
      ...
      request = OneLogin::RubySamlFrozen081::SloLogoutrequest.new(logout_request_document)

      assert request.is_valid?

      unauth_url = OneLogin::RubySamlFrozen081::SloLogoutresponse.new.create(settings, request.id)
    ...
    should "support additional params" do
      ...
      unauth_url = OneLogin::RubySamlFrozen081::SloLogoutresponse.new.create(settings, request.id, nil, { :hello => nil })
      assert unauth_url =~ /&hello=$/

      unauth_url = OneLogin::RubySamlFrozen081::SloLogoutresponse.new.create(settings, request.id, nil, { :foo => "bar" })
      assert unauth_url =~ /&foo=bar$/

      unauth_url = OneLogin::RubySamlFrozen081::SloLogoutresponse.new.create(settings, request.id, nil, { :RelayState => "http://idp.example.com" })
    ...
    should "set a custom successful logout message on the response" do
      ...
      request = OneLogin::RubySamlFrozen081::SloLogoutrequest.new(logout_request_document)
      unauth_url = OneLogin::RubySamlFrozen081::SloLogoutresponse.new.create(settings, request.id, "Custom Logout Message")
    ...
    context "when the settings indicate to sign (embebed) the logout response" do
      should "create a signed logout response" do
        ...
        request = OneLogin::RubySamlFrozen081::SloLogoutrequest.new(logout_request_document)
        params = OneLogin::RubySamlFrozen081::SloLogoutresponse.new.create_params(settings, request.id, "Custom Logout Message")
