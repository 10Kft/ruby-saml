...
class RubySamlFrozen081Test < Test::Unit::TestCase

  context "Response" do
    should "raise an exception when response is initialized with nil" do
      assert_raises(ArgumentError) { OneLogin::RubySamlFrozen081::Response.new(nil) }
    end
    ...
    should "adapt namespace" do
      response = OneLogin::RubySamlFrozen081::Response.new(response_document)
      assert_not_nil response.name_id
      response = OneLogin::RubySamlFrozen081::Response.new(response_document_2)
      assert_not_nil response.name_id
      response = OneLogin::RubySamlFrozen081::Response.new(response_document_3)
      assert_not_nil response.name_id
    end

    should "default to raw input when a response is not Base64 encoded" do
      decoded  = Base64.decode64(response_document_2)
      response = OneLogin::RubySamlFrozen081::Response.new(decoded)
      assert response.document
    end

    context "Assertion" do
      should "only retreive an assertion with an ID that matches the signature's reference URI" do
        response = OneLogin::RubySamlFrozen081::Response.new(wrapped_response_2)
    ...
    context "#validate!" do
      should "raise when encountering a condition that prevents the document from being valid" do
        response = OneLogin::RubySamlFrozen081::Response.new(response_document)
        assert_raise(OneLogin::RubySamlFrozen081::ValidationError) do
          response.validate!
        end
      end
    end
    ...
    context "#is_valid?" do
      should "return false when response is initialized with blank data" do
        response = OneLogin::RubySamlFrozen081::Response.new('')
      ...
      should "return true when the response is initialized with valid data" do
        response = OneLogin::RubySamlFrozen081::Response.new(response_document_4)
        response.stubs(:conditions).returns(nil)
        assert !response.is_valid?
        settings = OneLogin::RubySamlFrozen081::Settings.new
        assert !response.is_valid?
      ...
      should "support dynamic namespace resolution on signature elements" do
        response = OneLogin::RubySamlFrozen081::Response.new(fixture("no_signature_ns.xml"))
        response.stubs(:conditions).returns(nil)
        settings = OneLogin::RubySamlFrozen081::Settings.new
        response.settings = settings
      ...
      should "validate ADFS assertions" do
        response = OneLogin::RubySamlFrozen081::Response.new(fixture(:adfs_response_sha256))
        response.stubs(:conditions).returns(nil)
        settings = OneLogin::RubySamlFrozen081::Settings.new
        settings.idp_cert_fingerprint = "28:74:9B:E8:1F:E8:10:9C:A8:7C:A9:C3:E3:C5:01:6C:92:1C:B4:BA"
        response.settings = settings
      ...
      should "validate the digest" do
        response = OneLogin::RubySamlFrozen081::Response.new(r1_response_document_6)
        response.stubs(:conditions).returns(nil)
        settings = OneLogin::RubySamlFrozen081::Settings.new
      ...
      should "validate SAML 2.0 XML structure" do
        ...
        response = OneLogin::RubySamlFrozen081::Response.new(Base64.encode64(resp_xml))
        response.stubs(:conditions).returns(nil)
        settings = OneLogin::RubySamlFrozen081::Settings.new
        settings.idp_cert_fingerprint = signature_fingerprint_1
        response.settings = settings
        assert_raises(OneLogin::RubySamlFrozen081::ValidationError, 'Digest mismatch'){ response.validate! }
      end

      should "validate encrypted assertion in a signed message" do
        response = OneLogin::RubySamlFrozen081::Response.new(signed_message_encrypted_and_unsigned_assertion)
        response.stubs(:conditions).returns(nil)
        settings = OneLogin::RubySamlFrozen081::Settings.new
      ...
      should "validate encrypted signed assertion in a signed message" do
        response = OneLogin::RubySamlFrozen081::Response.new(signed_message_encrypted_and_signed_assertion)
        response.stubs(:conditions).returns(nil)
        settings = OneLogin::RubySamlFrozen081::Settings.new
        settings.idp_cert_fingerprint = "EE:17:4E:FB:A8:81:71:12:0D:2A:78:43:BC:E7:0C:07:58:79:F4:F4"
        response.settings = settings
      ...
      should "validate encrypted signed assertion in an unsigned message" do
        response = OneLogin::RubySamlFrozen081::Response.new(unsigned_message_encrypted_and_signed_assertion)
        response.stubs(:conditions).returns(nil)
        settings = OneLogin::RubySamlFrozen081::Settings.new
        settings.idp_cert_fingerprint = "EE:17:4E:FB:A8:81:71:12:0D:2A:78:43:BC:E7:0C:07:58:79:F4:F4"
    ...
    context "#name_id" do
      ...
      should "be extractable from an OpenSAML response" do
        response = OneLogin::RubySamlFrozen081::Response.new(fixture(:open_saml))
        assert_equal "someone@example.org", response.name_id
      end

      should "be extractable from a Simple SAML PHP response" do
        response = OneLogin::RubySamlFrozen081::Response.new(fixture(:simple_saml_php))
        assert_equal "someone@example.com", response.name_id
      end
    end
    ...
    context "#check_conditions" do
      should "check time conditions" do
        ...
        response = OneLogin::RubySamlFrozen081::Response.new(response_document_6)
        assert response.send(:validate_conditions, true)
        time     = Time.parse("2011-06-14T18:25:01.516Z")
        Time.stubs(:now).returns(time)
        response = OneLogin::RubySamlFrozen081::Response.new(response_document_5)
        assert response.send(:validate_conditions, true)
      end
      ...
      should "optionally allow for clock drift" do
        ...
        response = OneLogin::RubySamlFrozen081::Response.new(response_document_5, :allowed_clock_drift => 0.515)
        assert !response.send(:validate_conditions, true)

        Time.stubs(:now).returns(Time.parse("2011-06-14T18:21:01Z"))
        response = OneLogin::RubySamlFrozen081::Response.new(response_document_5, :allowed_clock_drift => 0.516)
        assert response.send(:validate_conditions, true)
      end
    end
    ...
    context "#attributes" do
      ...
      should "not raise errors about nil/empty attributes for EncryptedAttributes" do
        response = OneLogin::RubySamlFrozen081::Response.new(response_document_7)
        assert_equal 'Demo', response.attributes["first_name"]
      end

      should "not raise on responses without attributes" do
        response = OneLogin::RubySamlFrozen081::Response.new(response_document_4)
        assert_equal OneLogin::RubySamlFrozen081::Attributes.new, response.attributes
      end

      context "#multiple values" do
        should "extract single value as string" do
          response = OneLogin::RubySamlFrozen081::Response.new(fixture(:response_with_multiple_attribute_values))
          assert_equal "demo", response.attributes[:uid]
        end
