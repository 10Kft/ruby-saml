...
class RubySamlFrozen081Test < Test::Unit::TestCase
  ...
  context "SloLogoutrequest" do
    should "raise an exception when response is initialized with nil" do
      assert_raises(ArgumentError) { OneLogin::RubySamlFrozen081::SloLogoutrequest.new(nil) }
    end

    context "#is_valid?" do
      should "return false when response is initialized with blank data" do
        request = OneLogin::RubySamlFrozen081::SloLogoutrequest.new('')
      ...
      should "should be idempotent when the response is initialized with invalid data" do
        request = OneLogin::RubySamlFrozen081::SloLogoutrequest.new(invalid_xml_response)
        assert !request.is_valid?
        assert !request.is_valid?
      end
      ...
      should "raise error for invalid xml" do
        logout_request = OneLogin::RubySamlFrozen081::SloLogoutrequest.new(invalid_xml_response)
        assert_raises(OneLogin::RubySamlFrozen081::ValidationError) { logout_request.validate! }
      end
    end
